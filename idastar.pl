...
idastar(Soglia, Soluzione):-
    iniziale(S),
    heuristics: f(0, S, F),
    %regole: actions(S, ListaAzioni),
    regole: testabili(S, ListaAzioni),
    write('Nuova soglia: '),
    writeln(Soglia),
...
dfs_aux(nodo(_,_,S),_,_,_,_,-1):- finale(S), !, writeln('Stato finale raggiunto').         

% Ricerca non eccede la soglia: si prosegue in profondità
dfs_aux(nodo(F,G,S), [Azione|AzioniTail], [Azione|Soluzione], Visitati, Soglia, NuovaSoglia):- 
    F =< Soglia,!,
    regole:trasforma(Azione, S, SNuovo),
       (
        member(SNuovo, Visitati) ->
        % SNuovo è già stato visitato, prova con un'altra azione dalla coda AzioniTail
        dfs_aux(nodo(F,G,S), AzioniTail, Soluzione, Visitati, Soglia, NuovaSoglia)
        ;
        % SNuovo non è stato acnora visitato, questo è un nuovo nodo e la ricerca in profondità prosegue
        ...
        writeln(GNuovo),
        */
        %regole: actions(SNuovo, AzioniNuove),
        regole: testabili(SNuovo, AzioniNuove),
...
dfs_aux(nodo(F,_,_),_,_,_,_,F).  


%trova il valore minimo in una lista (usato per calcolare la nuova soglia)
list_min([L|Ls], Min) :-
    list_min(Ls, L, Min).
